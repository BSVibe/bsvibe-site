...
async function createStripeCheckout(
  ...
  if (!params.stripePriceId) {
    throw new Error(
      'Missing stripe_price_id on price record. Configure it in the database after creating the Price in Stripe Dashboard.',
    );
  }
...
async function createTossCheckout(
  params: CheckoutParams,
): Promise<CheckoutResult> {
  if (!TOSS_SECRET_KEY) {
...
export async function handleStripeWebhook(
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      const userId =
        session.metadata?.bsvibe_user_id || session.client_reference_id;
    ...
    case 'invoice.paid': {
      ...
      if (userId) {
        await recordPayment({
          ...
          status: 'succeeded',
          description:
            invoice.description || `Stripe invoice ${invoice.number}`,
